import ibm_db


dsn_hostname = "b0aebb68-94fa-46ec-a1fc-1c999edb6187.c3n41cmd0nqnrk39u98g.databases.appdomain.cloud"
dsn_uid = "ryh97900"       
dsn_pwd = "HRGII0IhlDS5MOJt" 

dsn_driver = "{IBM DB2 ODBC DRIVER}"
dsn_database = "bludb"        
dsn_port = "31249"                
dsn_protocol = "TCPIP"            
dsn_security = "ssl"

# {
#   "connection": {
#     "cli": {
#       "arguments": [
#         [
#           "-u",
#           "ryh97900",
#           "-p",
#           "HRGII0IhlDS5MOJt",
#           "--ssl",
#           "--sslCAFile",
#           "2ce1ab40-7379-45d8-a40c-38dcafaf95d0",
#           "--authenticationDatabase",
#           "admin",
#           "--host",
#           "b0aebb68-94fa-46ec-a1fc-1c999edb6187.c3n41cmd0nqnrk39u98g.databases.appdomain.cloud:31249"
#         ]
#       ],
#       "bin": "db2",
#       "certificate": {
#         "certificate_base64": "",
#         "name": "2ce1ab40-7379-45d8-a40c-38dcafaf95d0"
#       },
#       "composed": [
#         "db2 -u ryh97900 -p HRGII0IhlDS5MOJt --ssl --sslCAFile 2ce1ab40-7379-45d8-a40c-38dcafaf95d0 --authenticationDatabase admin --host b0aebb68-94fa-46ec-a1fc-1c999edb6187.c3n41cmd0nqnrk39u98g.databases.appdomain.cloud:31249"
#       ],
#       "environment": {},
#       "type": "cli"
#     },
#     "db2": {
#       "authentication": {
#         "method": "direct",
#         "password": "HRGII0IhlDS5MOJt",
#         "username": "ryh97900"
#       },
#       "certificate": {
#         "certificate_base64": "",
#         "name": "2ce1ab40-7379-45d8-a40c-38dcafaf95d0"
#       },
#       "composed": [
#         "db2://ryh97900:HRGII0IhlDS5MOJt@b0aebb68-94fa-46ec-a1fc-1c999edb6187.c3n41cmd0nqnrk39u98g.databases.appdomain.cloud:31249/bludb?authSource=admin&replicaSet=replset"
#       ],
#       "database": "bludb",
#       "host_ros": [
#         "b0aebb68-94fa-46ec-a1fc-1c999edb6187.c3n41cmd0nqnrk39u98g.databases.appdomain.cloud:30306"
#       ],
#       "hosts": [
#         {
#           "hostname": "b0aebb68-94fa-46ec-a1fc-1c999edb6187.c3n41cmd0nqnrk39u98g.databases.appdomain.cloud",
#           "port": 31249
#         }
#       ],
#       "jdbc_url": [
#         "jdbc:db2://b0aebb68-94fa-46ec-a1fc-1c999edb6187.c3n41cmd0nqnrk39u98g.databases.appdomain.cloud:31249/bludb:user=<userid>;password=<your_password>;sslConnection=true;"
#       ],
#       "path": "/bludb",
#       "query_options": {
#         "authSource": "admin",
#         "replicaSet": "replset"
#       },
#       "replica_set": "replset",
#       "scheme": "db2",
#       "type": "uri"
#     }
#   },
#   "instance_administration_api": {
#     "deployment_id": "crn:v1:bluemix:public:dashdb-for-transactions:us-south:a/fc50b7ad89154867bf5cf18586419aca:ed04eda8-b889-43ca-b373-4337ee292e32::",
#     "instance_id": "crn:v1:bluemix:public:dashdb-for-transactions:us-south:a/fc50b7ad89154867bf5cf18586419aca:ed04eda8-b889-43ca-b373-4337ee292e32::",
#     "root": "https://api.db2.cloud.ibm.com/v5/ibm"
#   }
# }



